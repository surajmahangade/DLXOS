...
void Producer(mem_buffer *mc, int process_id) {
  ...
  // Printf("Producer %d: Trying to produce. Chars produced so far: %d, next char: %c\n", process_id, chars_produced, MESSAGE[chars_produced]);
  sem_wait(mc->empty);  // Wait for empty slot
  // Printf("Producer %d: Passed empty semaphore. Chars produced so far: %d\n", process_id, chars_produced);
  // int inserted;
  if (chars_produced < message_len) {
    ...
    sem_signal(mc->full);  // Signal that a new item is available
