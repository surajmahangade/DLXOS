	     ...
	     uint32 *trapArgs)
{
  ...
  if (cause & TRAP_TRAP_INSTR) {
    ...
    case TRAP_PROCESS_FORK:
      ...
      ihandle = ProcessRealFork(currentPCB);
      break;
    ...
    case TRAP_PAGEFAULT:
      ...
      break;
    case TRAP_ROP_ACCESS:  // Add this new case for read-only page access
      dbprintf('t', "Got a ROP access interrupt!\n");
      if (MemoryROPAccessHandler(currentPCB) != MEM_SUCCESS) {
        printf("ERROR: ROP access handler failed for process %d\n", GetCurrentPid());
        ProcessKill();
      }
      break;
    default:
