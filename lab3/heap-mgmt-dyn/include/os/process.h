...
typedef struct PCB {
  ...
  unsigned int	flags;
  uint32 heap_vaddr_start;    // Virtual address where heap starts (page 4)
  uint32 heap_vaddr_end;      // Virtual address where heap ends (can grow)
  uint32 heap_pages_mapped;   // Number of pages currently mapped to heap
