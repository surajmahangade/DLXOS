...
int DfsCacheAllocateSlot(int blocknum) {
  int i;
  int mru_slot = 0;
  uint32 mru_time = cache[0].timestamp;
  ...
  // Find MRU (Most Recently Used) - opposite of LRU
  for (i = 1; i < DFS_CACHE_NUM_SLOTS; i++) {
    if (cache[i].timestamp > mru_time) {
      mru_time = cache[i].timestamp;
      mru_slot = i;
    }
  }
  
  // Evict MRU slot
  if (cache[mru_slot].dirty) {
    if (DfsWriteBlockUncached(cache[mru_slot].blocknum, &cache[mru_slot].data) == DFS_FAIL) {
      return DFS_FAIL;
    }
  }
  
  cache[mru_slot].valid = 1;
  cache[mru_slot].dirty = 0;
  cache[mru_slot].blocknum = blocknum;
  cache[mru_slot].timestamp = cache_clock++;
