...
int DfsReadBlock(uint32 blocknum, dfs_block *b) {
  ...
  // Calculate integer parts manually to avoid float printing issues
// 1. Calculate the total operations to avoid divide-by-zero
...
// In pattern1/os/dfs.c - Modify DfsCacheAllocateSlot

int DfsCacheAllocateSlot(int blocknum) {
  int i;
  static int fifo_head = 0;  // FIFO pointer
  int evict_slot;
  ...
  // FIFO replacement - evict oldest inserted
  evict_slot = fifo_head;
  fifo_head = (fifo_head + 1) % DFS_CACHE_NUM_SLOTS;
  
  // Write back if dirty
  if (cache[evict_slot].dirty) {
    if (DfsWriteBlockUncached(cache[evict_slot].blocknum, &cache[evict_slot].data) == DFS_FAIL) {
      return DFS_FAIL;
    }
  }
  
  cache[evict_slot].valid = 1;
  cache[evict_slot].dirty = 0;
  cache[evict_slot].blocknum = blocknum;
  cache[evict_slot].timestamp = cache_clock++;
